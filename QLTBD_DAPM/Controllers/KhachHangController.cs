...
namespace ITShop.Controllers
{
    ...
    public class KhachHangController : Controller
    {
        ...
        {
            int maNguoiDung = Convert.ToInt32(_httpContextAccessor.HttpContext.User.Claims.FirstOrDefault(c => c.Type == "ID")?.Value);
            var datHang = _context.DatHang.Where(r => r.NguoiDungID == maNguoiDung).Include(d => d.NguoiDung).Include(d => d.TinhTrang).Include(d => d.DatHang_ChiTiet).ThenInclude(s => s.SanPham);
